...
var FALLBACK_BASE = "https://www.atv.com.tr";

function _getBaseUrl() {
  return fetch(CONFIG_URL)
    .then(function(r) { return r.json(); })
    .then(function(cfg) {
      var p = cfg.providers && cfg.providers["atv"];
...
function getStreams(tmdbId, mediaType, season, episode) {
  var label = mediaType === "tv"
    ? "[atv] S" + season + "E" + episode + " tmdb:" + tmdbId
    : "[atv] tmdb:" + tmdbId;
  console.log(label);
  ...
  return _getBaseUrl().then(function(base) {
    ...
    .then(function(html) {
      var streams = _parseStreams(html, "atv");

      // İçerik sayfasına gitmek için ilk post linkini bul
      var linkRe = /href=["'](https?:\/\/[^"']*atv[^"']+)["']/i;
      ...
      .then(function(html2) {
        return _parseStreams(html2, "atv");
      });
    });
  }).catch(function(err) {
    console.error("[atv] Hata:", err.message);
    return [];
  });
}
