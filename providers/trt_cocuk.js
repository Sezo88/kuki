...
var FALLBACK_BASE = "https://www.trtcocuk.net.tr";

function _getBaseUrl() {
  return fetch(CONFIG_URL)
    .then(function(r) { return r.json(); })
    .then(function(cfg) {
      var p = cfg.providers && cfg.providers["trt_cocuk"];
...
function getStreams(tmdbId, mediaType, season, episode) {
  var label = mediaType === "tv"
    ? "[trt_cocuk] S" + season + "E" + episode + " tmdb:" + tmdbId
    : "[trt_cocuk] tmdb:" + tmdbId;
  console.log(label);

  return _getBaseUrl().then(function(base) {
    var searchUrl = base + "/arama?q=" + encodeURIComponent(String(tmdbId));
    ...
    .then(function(html) {
      var streams = _parseStreams(html, "trt_cocuk");

      // İçerik sayfasına gitmek için ilk post linkini bul
      var linkRe = /href=["'](https?:\/\/[^"']*trt[_-]?cocuk[^"']+)["']/i;
      ...
      .then(function(html2) {
        return _parseStreams(html2, "trt_cocuk");
      });
    });
  }).catch(function(err) {
    console.error("[trt_cocuk] Hata:", err.message);
    return [];
  });
}
