...
var FALLBACK_BASE = "https://www.turkanime.co";

function _getBaseUrl() {
  return fetch(CONFIG_URL)
    .then(function(r) { return r.json(); })
    .then(function(cfg) {
      var p = cfg.providers && cfg.providers["turkanime"];
...
function getStreams(tmdbId, mediaType, season, episode) {
  var label = mediaType === "tv"
    ? "[turkanime] S" + season + "E" + episode + " tmdb:" + tmdbId
    : "[turkanime] tmdb:" + tmdbId;
  console.log(label);

  return _getBaseUrl().then(function(base) {
    var searchUrl = base + "/index.php?icerik=" + encodeURIComponent(String(tmdbId));
    ...
    .then(function(html) {
      var streams = _parseStreams(html, "turkanime");

      // İçerik sayfasına gitmek için ilk post linkini bul
      var linkRe = /href=["'](https?:\/\/[^"']*turkanime[^"']+)["']/i;
      ...
      .then(function(html2) {
        return _parseStreams(html2, "turkanime");
      });
    });
  }).catch(function(err) {
    console.error("[turkanime] Hata:", err.message);
    return [];
  });
}
