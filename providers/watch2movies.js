// watch2movies Provider for Nuvio
...
var FALLBACK_BASE = "https://watch2movies.biz";

function _getBaseUrl() {
  return fetch(CONFIG_URL)
    .then(function(r) { return r.json(); })
    .then(function(cfg) {
      var p = cfg.providers && cfg.providers["watch2movies"];
...
function getStreams(tmdbId, mediaType, season, episode) {
  var label = mediaType === "tv"
    ? "[watch2movies] S" + season + "E" + episode + " tmdb:" + tmdbId
    : "[watch2movies] tmdb:" + tmdbId;
  console.log(label);
  ...
  return _getBaseUrl().then(function(base) {
    ...
    .then(function(html) {
      var streams = _parseStreams(html, "watch2movies");

      // İçerik sayfasına gitmek için ilk post linkini bul
      var linkRe = /href=["'](https?:\/\/[^"']*watch2movies[^"']+)["']/i;
      ...
      .then(function(html2) {
        return _parseStreams(html2, "watch2movies");
      });
    });
  }).catch(function(err) {
    console.error("[watch2movies] Hata:", err.message);
    return [];
  });
}
