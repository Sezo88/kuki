...
var FALLBACK_BASE = "https://www.showtv.com.tr";

function _getBaseUrl() {
  return fetch(CONFIG_URL)
    .then(function(r) { return r.json(); })
    .then(function(cfg) {
      var p = cfg.providers && cfg.providers["showtv"];
...
function getStreams(tmdbId, mediaType, season, episode) {
  var label = mediaType === "tv"
    ? "[showtv] S" + season + "E" + episode + " tmdb:" + tmdbId
    : "[showtv] tmdb:" + tmdbId;
  console.log(label);

  return _getBaseUrl().then(function(base) {
    var searchUrl = base + "/arama?kelime=" + encodeURIComponent(String(tmdbId));
    ...
    .then(function(html) {
      var streams = _parseStreams(html, "showtv");

      // İçerik sayfasına gitmek için ilk post linkini bul
      var linkRe = /href=["'](https?:\/\/[^"']*showtv[^"']+)["']/i;
      ...
      .then(function(html2) {
        return _parseStreams(html2, "showtv");
      });
    });
  }).catch(function(err) {
    console.error("[showtv] Hata:", err.message);
    return [];
  });
}
