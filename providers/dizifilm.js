...
var FALLBACK_BASE = "https://dizifilm.to";

function _getBaseUrl() {
  return fetch(CONFIG_URL)
    .then(function(r) { return r.json(); })
    .then(function(cfg) {
      var p = cfg.providers && cfg.providers["dizifilm"];
...
function getStreams(tmdbId, mediaType, season, episode) {
  var label = mediaType === "tv"
    ? "[dizifilm] S" + season + "E" + episode + " tmdb:" + tmdbId
    : "[dizifilm] tmdb:" + tmdbId;
  console.log(label);
  ...
  return _getBaseUrl().then(function(base) {
    ...
    .then(function(html) {
      var streams = _parseStreams(html, "dizifilm");

      // İçerik sayfasına gitmek için ilk post linkini bul
      var linkRe = /href=["'](https?:\/\/[^"']*dizifilm[^"']+)["']/i;
      ...
      .then(function(html2) {
        return _parseStreams(html2, "dizifilm");
      });
    });
  }).catch(function(err) {
    console.error("[dizifilm] Hata:", err.message);
    return [];
  });
}
