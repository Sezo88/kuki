...
var FALLBACK_BASE = "https://www.inatbox.com";

function _getBaseUrl() {
  return fetch(CONFIG_URL)
    .then(function(r) { return r.json(); })
    .then(function(cfg) {
      var p = cfg.providers && cfg.providers["inatbox"];
...
function getStreams(tmdbId, mediaType, season, episode) {
  var label = mediaType === "tv"
    ? "[inatbox] S" + season + "E" + episode + " tmdb:" + tmdbId
    : "[inatbox] tmdb:" + tmdbId;
  console.log(label);
  ...
  return _getBaseUrl().then(function(base) {
    ...
    .then(function(html) {
      var streams = _parseStreams(html, "inatbox");

      // İçerik sayfasına gitmek için ilk post linkini bul
      var linkRe = /href=["'](https?:\/\/[^"']*inatbox[^"']+)["']/i;
      ...
      .then(function(html2) {
        return _parseStreams(html2, "inatbox");
      });
    });
  }).catch(function(err) {
    console.error("[inatbox] Hata:", err.message);
    return [];
  });
}
