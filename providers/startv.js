...
var FALLBACK_BASE = "https://www.startv.com.tr";

function _getBaseUrl() {
  return fetch(CONFIG_URL)
    .then(function(r) { return r.json(); })
    .then(function(cfg) {
      var p = cfg.providers && cfg.providers["startv"];
...
function getStreams(tmdbId, mediaType, season, episode) {
  var label = mediaType === "tv"
    ? "[startv] S" + season + "E" + episode + " tmdb:" + tmdbId
    : "[startv] tmdb:" + tmdbId;
  console.log(label);

  return _getBaseUrl().then(function(base) {
    var searchUrl = base + "/search?q=" + encodeURIComponent(String(tmdbId));
    ...
    .then(function(html) {
      var streams = _parseStreams(html, "startv");

      // İçerik sayfasına gitmek için ilk post linkini bul
      var linkRe = /href=["'](https?:\/\/[^"']*startv[^"']+)["']/i;
      ...
      .then(function(html2) {
        return _parseStreams(html2, "startv");
      });
    });
  }).catch(function(err) {
    console.error("[startv] Hata:", err.message);
    return [];
  });
}
