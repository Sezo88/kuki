...
var FALLBACK_BASE = "https://yts.mx";

function _getBaseUrl() {
  return fetch(CONFIG_URL)
    .then(function(r) { return r.json(); })
    .then(function(cfg) {
      var p = cfg.providers && cfg.providers["yts"];
...
function getStreams(tmdbId, mediaType, season, episode) {
  var label = mediaType === "tv"
    ? "[yts] S" + season + "E" + episode + " tmdb:" + tmdbId
    : "[yts] tmdb:" + tmdbId;
  console.log(label);

  return _getBaseUrl().then(function(base) {
    var searchUrl = base + "/api/v2/list_movies.json?query_term=" + encodeURIComponent(String(tmdbId));
    ...
    .then(function(html) {
      var streams = _parseStreams(html, "yts");

      // İçerik sayfasına gitmek için ilk post linkini bul
      var linkRe = /href=["'](https?:\/\/[^"']*yts[^"']+)["']/i;
      ...
      .then(function(html2) {
        return _parseStreams(html2, "yts");
      });
    });
  }).catch(function(err) {
    console.error("[yts] Hata:", err.message);
    return [];
  });
}
