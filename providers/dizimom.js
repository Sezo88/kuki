...
var FALLBACK_BASE = "https://dizimom.pro";

function _getBaseUrl() {
  return fetch(CONFIG_URL)
    .then(function(r) { return r.json(); })
    .then(function(cfg) {
      var p = cfg.providers && cfg.providers["dizimom"];
...
function getStreams(tmdbId, mediaType, season, episode) {
  var label = mediaType === "tv"
    ? "[dizimom] S" + season + "E" + episode + " tmdb:" + tmdbId
    : "[dizimom] tmdb:" + tmdbId;
  console.log(label);
  ...
  return _getBaseUrl().then(function(base) {
    ...
    .then(function(html) {
      var streams = _parseStreams(html, "dizimom");

      // İçerik sayfasına gitmek için ilk post linkini bul
      var linkRe = /href=["'](https?:\/\/[^"']*dizimom[^"']+)["']/i;
      ...
      .then(function(html2) {
        return _parseStreams(html2, "dizimom");
      });
    });
  }).catch(function(err) {
    console.error("[dizimom] Hata:", err.message);
    return [];
  });
}
