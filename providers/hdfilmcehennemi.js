// hdfilmcehennemi Provider for Nuvio
...
var FALLBACK_BASE = "https://www.hdfilmcehennemi.nl";

function _getBaseUrl() {
  return fetch(CONFIG_URL)
    .then(function(r) { return r.json(); })
    .then(function(cfg) {
      var p = cfg.providers && cfg.providers["hdfilmcehennemi"];
...
function getStreams(tmdbId, mediaType, season, episode) {
  var label = mediaType === "tv"
    ? "[hdfilmcehennemi] S" + season + "E" + episode + " tmdb:" + tmdbId
    : "[hdfilmcehennemi] tmdb:" + tmdbId;
  console.log(label);

  return _getBaseUrl().then(function(base) {
    var searchUrl = base + "/search/" + encodeURIComponent(String(tmdbId));
    ...
    .then(function(html) {
      var streams = _parseStreams(html, "hdfilmcehennemi");

      // İçerik sayfasına gitmek için ilk post linkini bul
      var linkRe = /href=["'](https?:\/\/[^"']*hdfilmcehennemi[^"']+)["']/i;
      ...
      .then(function(html2) {
        return _parseStreams(html2, "hdfilmcehennemi");
      });
    });
  }).catch(function(err) {
    console.error("[hdfilmcehennemi] Hata:", err.message);
    return [];
  });
}
