...
var FALLBACK_BASE = "https://izle.ai";

function _getBaseUrl() {
  return fetch(CONFIG_URL)
    .then(function(r) { return r.json(); })
    .then(function(cfg) {
      var p = cfg.providers && cfg.providers["izleai"];
...
function getStreams(tmdbId, mediaType, season, episode) {
  var label = mediaType === "tv"
    ? "[izleai] S" + season + "E" + episode + " tmdb:" + tmdbId
    : "[izleai] tmdb:" + tmdbId;
  console.log(label);
  ...
  return _getBaseUrl().then(function(base) {
    ...
    .then(function(html) {
      var streams = _parseStreams(html, "izleai");

      // İçerik sayfasına gitmek için ilk post linkini bul
      var linkRe = /href=["'](https?:\/\/[^"']*izleai[^"']+)["']/i;
      ...
      .then(function(html2) {
        return _parseStreams(html2, "izleai");
      });
    });
  }).catch(function(err) {
    console.error("[izleai] Hata:", err.message);
    return [];
  });
}
